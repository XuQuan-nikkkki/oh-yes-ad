...
export async function PATCH(req: NextRequest, context: RouteContext) {
  ...
  const existing = await prisma.projectReceivableActualNode.findUnique({
    where: { id },
    select: { id: true },
  });
  ...
  const updated = await prisma.projectReceivableActualNode.update({
    where: { id },
    data: patchData,
...
export async function DELETE(_req: NextRequest, context: RouteContext) {
  ...
  await prisma.projectReceivableActualNode.delete({
    where: { id },
  });
