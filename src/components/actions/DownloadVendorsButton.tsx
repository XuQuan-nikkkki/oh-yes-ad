...
import type { Vendor } from "@/components/VendorsTable";
...
const DownloadVendorsButton = ({ vendors, disabled = false }: Props) => {
    ...
    try {
      ...
      worksheet.columns = columns.map((column, index) => {
        ...
        const width = Math.min(
          Math.max(baseLength + 2, 12),
          index === columns.length - 1 ? 40 : 30,
        );
        return { width };
      });
      ...
      link.href = url;
      link.download = `供应商列表-${yyyy}-${mm}-${dd}.xlsx`;
...
export default DownloadVendorsButton;
