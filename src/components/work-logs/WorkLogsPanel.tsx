...
export default function WorkLogsPanel() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [
        ...
        fetch("/api/planned-work-entries?projectType=all", {
          cache: "no-store",
        }),
        fetch("/api/actual-work-entries?projectType=all", {
          cache: "no-store",
        }),
      ...
      const [me, planned, actual, employeeList, projects, adjustments, leaves] =
        await Promise.all([
          meRes.json(),
  ...
  const totalActualHours = useMemo(() => {
    return weekActualRows.reduce((sum, row) => {
      const hours = Math.max(
        dayjs(row.endDate).diff(dayjs(row.startDate), "minute") / 60,
        0,
      );
      return sum + hours;
    }, 0);
  ...
  const weekActualBoard = useMemo<WeekBoardColumn[]>(() => {
    ...
    const columnMap = new Map(
      columns.map((column) => [column.dateKey, column]),
    );
  ...
  const openPlatformLogModal = () => {
    ...
    const preferredProjectId =
      workLogProjectOptions.find((project) => project.name.includes("中台"))
        ?.id ??
  ...
  const handleDeleteWorkLog = async (entry: ActualWorkEntryRow) => {
    try {
      const res = await fetch(
        `/api/actual-work-entries/${entry.id}?projectType=all`,
        {
          method: "DELETE",
        },
      );
    ...
    <Space orientation="vertical" size={16} style={{ width: "100%" }}>
      {loading ? (
        ...
        <>
                  ...
                  <Col xs={24} sm={12} lg={8} key={task.taskId}>
                    <Card
                      ...
                      title={
                        <span>
                          任务：
                          <AppLink href={`/project-tasks/${task.taskId}`}>
                            {task.taskName}
                          </AppLink>
                        </span>
                      }
          ...
          <Card
            title={`实际工时（本周，总计 ${Number(totalActualHours.toFixed(2))} 小时）`}
            type="inner"
          >
              ...
              <div style={{ overflowX: "auto", paddingBottom: 4 }}>
                <div
                  style={{
                    display: "flex",
                    gap: 12,
                    minWidth: weekActualBoard.length * 240,
                  }}
                >
                            ...
                            <Card
                              ...
                              actions={[
                                ...
                                <Popconfirm
                                  ...
                                  cancelText="取消"
                                  onConfirm={() =>
                                    void handleDeleteWorkLog(entry)
                                  }
                                >
                                  <span
                                    role="button"
